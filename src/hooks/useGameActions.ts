...
export function useGameActions(
  ...
  useEffect(() => {
    ...
    const handlePickTarget = (ev: any) => {
      ...
      setGameState(prev => {
        const sel = (prev as any).pendingAbilitySelect;
        if (!sel || sel.type !== 'corruption_steal') return prev;
        return {
          ...prev,
          pendingAbilitySelect: { ...sel, targetUid: uid },
        } as GameState;
      });
    ...
    const handleRequestRoll = (ev: any) => {
      try {
        ...
        setGameState(prev => {
          ...
          try { resolveQueue(newState as any, events); } catch (e) { console.error('resolveQueue error in handleRequestRoll', e); }
    ...
    const handleMaulwurfRequestRoll = (ev: any) => {
      try {
        ...
        setGameState(prev => {
          ...
          try { resolveQueue(newState as any, events); } catch (e) { console.error('resolveQueue error in handleMaulwurfRequestRoll', e); }
          if (afterQueueResolved) afterQueueResolved();
          return { ...newState, _effectQueue: [] };
        });
      } catch (e) {
    ...
    const handleMaulwurfCancel = () => {
      setGameState(prev => ({
        ...prev,
        pendingAbilitySelect: undefined,
      } as GameState));
      try {
        const modal = document.getElementById('pc-maulwurf-modal');
  ...
  const playCard = useCallback((player: Player, handIndex: number, lane?: 'innen' | 'aussen') => {
    ...
    setGameState(prev => {
      ...
      const result = ((): GameState => {
        ...
        log(`💳 AP Counter: ${ cardName } (${ cardType }) kostet ${ cost } AP | Vorher: ${ prevAp } AP → Nachher: ${ prevAp - cost } AP`);
        ...
        if (playedCard.kind === 'pol') {
          ...
          if (isInitiative) {
            ...
            if (typeStr.includes('sofort')) {
              ...
              if (newState._effectQueue.length > 0) {
                resolveQueue(newState, [...newState._effectQueue]);
                newState._effectQueue = [];
              }
              return newState;
